...
pub fn member_2(_vm: &vm::Machine, _process: &mut Process, args: &[Term]) -> bif::Result {
...
// fn lists_reverse_alloc(process: &mut Process, mut list: Term, mut tail: Term) -> bif::Result {
...
pub fn reverse_2(_vm: &vm::Machine, process: &mut Process, args: &[Term]) -> bif::Result {
    ...
    Ok(cons
        .iter()
        .fold(args[1], |acc, val| cons!(&process.heap, *val, acc)))
...
pub fn keymember_3(_vm: &vm::Machine, process: &mut Process, args: &[Term]) -> bif::Result {
...
pub fn keysearch_3(_vm: &vm::Machine, process: &mut Process, args: &[Term]) -> bif::Result {
    keyfind(keyfind_3, process, args).map(|res| {
        if res.is_tuple() {
            return tup2!(&process.heap, atom!(VALUE), res);
        }
        res
    })
}

pub fn keyfind_3(_vm: &vm::Machine, process: &mut Process, args: &[Term]) -> bif::Result {
...
fn keyfind(_func: bif::Fn, _process: &mut Process, args: &[Term]) -> bif::Result {
